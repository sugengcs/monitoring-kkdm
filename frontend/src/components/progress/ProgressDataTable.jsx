...
const ProgressDataTable = ({ lahanData, onEdit, onDelete, editingItem, editForm, onEditSave, onEditCancel }) => {
  ...
  return (
    ...
    >
      {/* Table header */}
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 p-4 pb-0 flex-shrink-0">
        <div>
          <h2 className="text-base font-bold text-white tracking-tight" style={{ textShadow: '0 0 20px rgba(59,130,246,0.3)' }}>Tabel Data Progress</h2>
          <p className="text-xs text-slate-400 mt-1">Detail data progress lahan</p>
        </div>
        <div className="flex items-center gap-3">
          ...
          <div className="relative">
            ...
            <input
              ...
              className="pl-10 pr-4 py-2.5 rounded-xl text-sm text-white bg-white/5 border border-white/10 focus:outline-none focus:border-cyan-500/50 focus:shadow-lg focus:shadow-cyan-500/20 w-48 transition-all"
      ...
      {/* Table */}
      <div className="overflow-x-auto p-4 flex-1 min-h-0">
        <table className="w-full text-sm">
            ...
            <tr style={{ borderBottom: '2px solid rgba(59,130,246,0.2)', background: 'rgba(59,130,246,0.05)' }}>
              {[
                ...
                <th
                  ...
                  className="text-left text-slate-300 font-semibold px-3 py-3 uppercase tracking-wider text-[10px] cursor-pointer hover:text-white hover:bg-cyan-500/10 transition-colors"
                >
              ...
              <th className="text-center text-slate-300 font-semibold px-3 py-3 uppercase tracking-wider text-[10px]">Aksi</th>
            </tr>
          </thead>
          <tbody>
            {paginatedData.length === 0 ? (
              ...
              paginatedData.map((item, idx) => {
                ...
                return (
                  ...
                  >
                    <td className="px-3 py-2.5 text-slate-400 font-medium text-xs">{item.no}</td>
                    <td className="px-3 py-2.5">
                      {isEditing ? (
                        <input
                          ...
                          className="w-full px-2 py-1 rounded-lg text-xs text-white bg-white/10 border border-white/20 focus:outline-none focus:border-cyan-500 transition-all"
                        />
                      ) : (
                        <span className="text-white font-semibold text-xs">{item.lokasi}</span>
                      )}
                    </td>
                    <td className="px-3 py-2.5">
                      {isEditing ? (
                        <input
                          ...
                          className="w-full px-2 py-1 rounded-lg text-xs text-white bg-white/10 border border-white/20 focus:outline-none focus:border-cyan-500 font-mono transition-all"
                        />
                      ) : (
                        <span className="text-slate-300 font-mono text-xs">{item.kebutuhan} m²</span>
                      )}
                    </td>
                    <td className="px-3 py-2.5">
                      {isEditing ? (
                        ...
                        <span className="text-cyan-400 font-mono font-semibold text-xs">{item.realisasi} m²</span>
                      )}
                    </td>
                    <td className="px-3 py-2.5">
                      {isEditing ? (
                        ...
                        <span className="text-slate-300 font-mono text-xs">{item.sisa} m²</span>
                      )}
                    </td>
                    <td className="px-3 py-2.5">
                      {isEditing ? (
                        <span className="text-slate-500 text-[10px] italic">Auto-calculated</span>
                      ) : (
                        <div className="flex items-center gap-2">
                          ...
                          <span className="text-white font-bold w-12 text-right text-xs">{progress.toFixed(1)}%</span>
                        </div>
                      )}
                    </td>
                    <td className="px-3 py-2.5">
                      {isEditing ? (
                        ...
                        <span className="text-slate-500 text-[10px]">{item.keterangan}</span>
                      )}
                    </td>
                        ...
                        {isEditing ? (
                          <>
                            <button
                              ...
                              className="p-1.5 rounded-lg text-green-400 hover:bg-green-500/20 transition-all hover:scale-110"
                            >
                              <Save className="w-3.5 h-3.5" />
                            </button>
                            <button
                              onClick={() => onEditCancel?.()}
                              className="p-1.5 rounded-lg text-red-400 hover:bg-red-500/20 transition-all hover:scale-110"
                            >
                          ...
                          <>
                            <button
                              ...
                              className="p-1.5 rounded-lg text-cyan-400 hover:bg-cyan-500/20 transition-all hover:scale-110"
                            >
          ...
          <tfoot>
            <tr style={{ 
              ...
              <td className="px-3 py-3 font-bold text-cyan-400 text-xs">TOTAL</td>
              <td className="px-3 py-3"></td>
              <td className="px-3 py-3">
                <span className="text-white font-bold text-xs">{sortedData.reduce((sum, item) => {
      ...
      {/* Pagination */}
      <div className="flex items-center justify-between px-4 py-3 border-t border-white/06 flex-shrink-0">
        ...
        <div className="flex items-center gap-1">
          <button
            ...
            className="p-1.5 rounded-lg text-white bg-white/5 border border-white/10 disabled:opacity-50 disabled:cursor-not-allowed transition-all hover:bg-white/10 disabled:hover:bg-white/5"
          >
